...
function vocab = build_col_vocabulary( image_paths, vocab_size )
...
% centroid / visual word.
    ...
    sift_features = cell(num_images, 3);
    step_size=5;
    ...
    for i = 1:num_images
        ...
        % Extract SIFT features for each color channel. SHOULD PROBABLY
        % CHANGE FROM FAST
        [~, sift_features{i, 1}] = vl_dsift(r, 'step', step_size, 'Fast');
        [~, sift_features{i, 2}] = vl_dsift(g, 'step', step_size, 'Fast');
        [~, sift_features{i, 3}] = vl_dsift(b, 'step', step_size, 'Fast');
    end
    disp("starting kmeans")
    % Combine SIFT features from all color channels for k-means clustering
    all_sift_features = cat(2, sift_features{:});
    [vocab, ~] = vl_kmeans(double(all_sift_features), vocab_size);
end
