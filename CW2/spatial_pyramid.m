...
function image_feats = spatial_pyramid(image_paths, num_layers, step_size, tfidf_weighted, num_words, weight,vocab_file_name)
    ...
    for i = 1:num_images
        ...
        for j = 1:num_layers
            ...
            region_feats = extract_sift_features(pyramid{j}, step_size,vocab_file_name);
...
function histogram = extract_sift_features(images, step_size,vocab_file_name)
    ...
    histograms = zeros(num_images, num_words); 
    
    for i = 1:num_images
        [locations, feats] = vl_dsift(single(images{i}), 'step', step_size, 'Fast');
        distances = vl_alldist2(single(feats), single(vocab.vocab));
...
function features = get_tfidf_weights(features, step_size, num_images)
  ...
  for col_start = 1:step_size:num_cols
    ...
    col_end = min(col_start + step_size - 1, num_cols);
    %calculate apply scaling factor to the block
