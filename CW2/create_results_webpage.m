...
% Normalize confusion matrix for displaying proportions
confusion_matrix_normalized = confusion_matrix ./ sum(confusion_matrix, 2);

fig_handle = figure; 
imagesc(confusion_matrix_normalized); 
set(fig_handle, 'Color', [.988, .988, .988])
axis_handle = get(fig_handle, 'CurrentAxes');
set(axis_handle, 'XTick', 1:num_categories)
set(axis_handle, 'XTickLabel', categories)
set(axis_handle, 'YTick', 1:num_categories)
set(axis_handle, 'YTickLabel', categories)
xlabel('Predicted Category');
ylabel('True Category');
colorbar;

% OUR NEW CODE THAT ADDS NUMBERS TO THE MATRIX:
for i = 1:num_categories
    for j = 1:num_categories

            text(j, i, sprintf('%.2f', confusion_matrix_normalized(i, j)), 'HorizontalAlignment', 'center','fontSize',6);
